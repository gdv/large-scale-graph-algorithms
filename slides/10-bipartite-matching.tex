...
\begin{align}
  ...
  \sum_{w\in T} e_{v,w} = 1 \quad& \forall v\in S           \notag\\
  \sum_{v\in S} e_{v,w} = 1 \quad& \forall w\in T           \notag\\
  e_{v,w}\in \{0,1\}         \quad& \forall (v,w)\in E \notag
\end{align}
\end{frame}
...
\begin{block}{Primal}
\begin{align}
  \min \sum_{(v,w)\in E} c_{v,w}e_{v,w}                  \quad&\text{subject to }\notag\\
  \sum_{w\in N(v)} e_{v,w} = 1 \quad& \forall v\in V           \notag\\
...
\begin{algorithm}[H]
  ...
  $M\gets \emptyset$, $l=\bar{0}$\;
    ...
    \eIf{$R_{T}\cap Z\neq\emptyset$}{
      ...
      $\Delta = \min_{s\in S\cap Z, t\in T\setminus Z} \left\{ c(s,t) - l(s) - l(t)\right\}$\;
      \ForEach{$v\in V$}{
...
\begin{algorithm}[H]
  ...
  $Z\gets\{j\}$\;
  $\Delta = \min_{t\in T\setminus Z} \left\{ c(j,t) - l(j) - l(t)\right\}$\;
  $x$ is the argmin\;
  \ForEach{$v\in V$}{
    \lIf{$v\in S\cap Z$}{$l(v) \gets l(v) + \Delta$}
    \lIf{$v\in T\cap Z$}{$l(v) \gets l(v) - \Delta$}
