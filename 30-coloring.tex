...
\begin{algorithm}[H]
\KwData{graph \g}
\(t_{u,v}\gets 1 \ \forall u\neq v\in V\), \(k\gets n\), \(B\gets\) single vertices\tcc{\(t_{u,v}\): trail matrix}
\While{not stopping condition}{
\(\delta_{u,v}\gets 0\ \forall u\neq v\in V\)\tcc{\(\delta_{u,v}\) is the update matrix}
  \ForEach{ant \(a\)}{
    ...
    \If{\(S\) is a partial solution}{
      ...
      \(Pr[v,i] = f_{v,i}^{\alpha}g_{v,i}^{\beta} / \sum_{u} f_{u,i}^{\alpha}g_{u,i}^{\beta}\)
    }
